...
use ckb_types::{
    ...
    packed::{self, CellInput, CellOutput, OutPoint, Script, ScriptBuilder},
    prelude::*,
};
use ckb_vm::{
...
fn _test_load_cell_all(data: &[u8]) -> Result<(), TestCaseError> {
    ...
    let rtx = Arc::new(ResolvedTransaction {
        transaction: TransactionBuilder::default().build(),
        resolved_cell_deps: vec![],
        resolved_inputs: vec![input_cell.clone()],
        resolved_dep_groups: vec![],
    });
...
fn _test_load_header(
    ...
    let data_hash: packed::Byte32 = blake2b_256(data).into();
...
fn _test_load_witness(data: &[u8], source: SourceEntry) -> Result<(), TestCaseError> {
    ...
    let witness: packed::Bytes = Bytes::from(data.to_owned()).into();

    let witness_correct_data = witness.raw_data();

    let witnesses = vec![witness];
    let rtx = Arc::new(ResolvedTransaction {
        transaction: TransactionBuilder::default().witnesses(witnesses).build(),
        resolved_cell_deps: vec![],
        resolved_inputs: vec![],
        resolved_dep_groups: vec![],
    });
...
fn _test_load_group_witness(data: &[u8], source: SourceEntry) -> Result<(), TestCaseError> {
    ...
    let dummy_witness = Bytes::default().into();
...
fn _test_load_cell_data_as_code(
    ...
    let rtx = Arc::new(ResolvedTransaction {
        transaction: TransactionBuilder::default().output_data(&data).build(),
...
fn test_load_overflowed_cell_data_as_code() {
    ...
    assert!(matches!(result.unwrap_err(), VMError::MemOutOfBound(..)));
}
...
fn _test_load_input(
    ...
    let data = Bytes::from(data.to_owned());
    let hash: packed::Byte32 = blake2b_256(&data).into();
    ...
    let rtx = Arc::new(ResolvedTransaction {
        transaction: TransactionBuilder::default().input(input.clone()).build(),
        resolved_cell_deps: vec![],
        resolved_inputs: vec![],
        resolved_dep_groups: vec![],
    });
